...
// render (this function is built on the "display_update" funtion above)
void render_display(uint8_t *array) {
	...
	for(i = 0; i < 4; i++) {
		...
		for(j = 0; j < 128; j++) {
			spi_send_recv(array[i*128 + j]); //updates all pixels
		}
	}
}

//clears the screen (not updating)
//sets all bits to zero
void clear_display(uint8_t *array){
	int i;
	for(i = 0; i < 4 * 128; i++){
		array[i] = 0;
	}
